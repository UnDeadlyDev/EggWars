...
public class Starting {
            ...
            public void run() {
                ...
                for (EwPlayer ewplayer1 : arenaIn.getPlayers()) {
                    ...
                    if (countDown == 5 || countDown == 4 || countDown == 3 || countDown == 2 || countDown == 1) {
                        ReflectionUtils.sendTitle(ewplayer1.getPlayer(), 0, 10, 30, TranslationUtils.getMessage("gameplay.lobby.starting.title", ewplayer1.getPlayer()).replace("<time>", Integer.toString(countDown)), TranslationUtils.getMessage("gameplay.lobby.starting.subtitle", ewplayer1.getPlayer()));
                    }
    ...
    public static void releasePlayersAndStartGame(Arena arena) {
        ...
        /*if (arena.getStatus().isGame()) {
            ...
            if ((winner != null) || arena.getAliveTeams().isEmpty()) {
                Finish.finish(arena, winner);
                return;
            }
        }*/
        if (arena.getReleaseCountdown() != 0 || arena.skipSoloLobby()) {
        ...
        for (EwPlayer ewplayer : arena.getAlivePlayers()) {
            ReflectionUtils.sendTitle(ewplayer.getPlayer(), Integer.valueOf(5), Integer.valueOf(10), Integer.valueOf(5), TranslationUtils.getMessage("gameplay.lobby.go", ewplayer.getPlayer()), " ");
